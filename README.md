...
Currently, our model's performance does not align with the expectations set by the referenced paper. The achieved results significantly deviate from those reported in the paper, displaying lower levels of accuracy. During training, minimal improvement in accuracy was observed, and the model exhibits a notable occurrence of false positives. The outcomes generated by our model are characterized by consistently low confidence scores, indicating a lack of certainty in the predictions.

## Conclusion
Despite investing significant hours and effort into the project, we regrettably did not achieve the results reported in the paper. Even though we followed each step described in the paper and did extra research on the subject.

We have to mention the fault is not fully in the paper. We acknowledge that potential mistakes in our coding, particularly during the training process, may have contributed to the differences between our outcomes and those presented in the paper. Despite our best effort, we were unable to replicate the reported performance levels.
